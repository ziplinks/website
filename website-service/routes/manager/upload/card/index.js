...
module.exports = app => {
    ...
    const upload = multer({
        dest: __dirname + "/../../../../uploads/card"//上传的图片存储路径
    });
    app.post('/backstage/upload/card', authMiddleWare, upload.single('file'), async (req, res) => {
        const file = req.file; //req上本来是没有file属性的 通过multer这个中间件实现的 upload.single代表单张图片
        file.url = `http://localhost:3000/uploads/card/${file.filename}`;
