...
import {ElMessage, type FormInstance, type FormRules, type UploadInstance, ElMessageBox} from "element-plus";
import {GetTableData} from "@/api/home/types/home";
import {getToken} from "@/utils/cache/cookies";
import {createHorizontalCard, getCardHorizontalList, updateCardHorizontalDataApi} from "@/api/home";
  ...
  formRef.value?.validate((valid: boolean, fields) => {
    if (valid) {
      let tag =  formData.cardList.some(v=>{
        return !v.imgUrl || !v.titleFirst || !v.titleFirstHighlight || !v.titleSecondHighlight || !v.titleSecond || !v.desc
      })
      if(tag){
      ...
      if (!formData._id) {
        delete formData._id
        createHorizontalCard(formData)
          .then(() => {
            ElMessage.success("新增成功")
          })
      } else {
        updateCardHorizontalDataApi(formData)
          .then(() => {
...
<!--                        {{scope.$index}}-->
                  ...
                  <el-table-column prop="titleFirst" label="第一行标题" align="center">
                    <template #default="scope">
                      <el-input maxlength="6" v-model="scope.row.titleFirst"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="titleFirstHighlight" label="第一行标题高亮" align="center">
                    <template #default="scope">
                      <el-input maxlength="6" v-model="scope.row.titleFirstHighlight"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="titleSecond" label="第二行标题" align="center">
                    <template #default="scope">
                      <el-input maxlength="6" v-model="scope.row.titleSecond"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="titleSecondHighlight" label="第二行标题高亮" align="center">
                    <template #default="scope">
                      <el-input maxlength="6" v-model="scope.row.titleSecondHighlight"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column prop="color" label="高亮颜色" align="center">
                    <template #default="scope">
